    ...
    if len(all_links) > 0:
        ...
        if ignore_texts == False:
            ...
            if _data == []:
                ...
                data_contents = _data[0].contents[0]
...
def switcher(mode = None,ignore_texts = None):
    """
    - mode = 0: 将图片和文件分开下载
...
if __name__ == '__main__':
    ...
    jutils(mode = mode)
    username = re.findall(re.compile(r'\n+(.*?)\n+'), BeautifulSoup(requests.get(all_links[0]).content.decode('utf-8'), 'lxml').select('.post__user-name')[0].contents[0])[0].replace(' ', '')
    try:
    ...
    threads__ = []
    for i in range(n):
        thread = threading.Thread(target = switcher, args = (mode, ignore_texts))
